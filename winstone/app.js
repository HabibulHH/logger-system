// app.js — a simple Express API that demonstrates the winston logger in a
...
app.get('/students', (req, res) => {
  req.log.debug('listing all students', { count: students.length });
  res.json(students);
});
...
app.get('/students/:id', (req, res) => {
  ...
  if (!student) {
    req.log.warn('student not found', { id });
    return res.status(404).json({ error: 'student not found' });
  }

  req.log.info('student found', { id, name: student.name });
  res.json(student);
});
...
app.use((err, req, res, next) => {
  req.log.error('request failed', { err: err.message, stack: err.stack });
  res.status(500).json({ error: 'internal server error' });
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  logger.info('server listening', { port: PORT, level: logger.level });
});
